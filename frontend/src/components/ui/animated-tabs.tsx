...
const TabsTrigger = ({ value, children }: TabsTriggerProps) => {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="relative px-4 py-2 w-[50%] text-sm font-medium rounded-t-md overflow-hidden transition-colors duration-500 cursor-pointer"
    >
      <span
        className={twMerge(
      ...
      <span
        className={twMerge(
          ...
          isActive ? "scale-y-100 bg-purple-400 dark:bg-white" : "bg-transparent"
        )}
      />
    </button>
  );
...
const TabsContent = ({ value, children }: TabsContentProps): ReactElement => {
  ...
  return (
    <div
      ...
      style={{ 
        ...
        // Ngăn component trigger các side effects khi ẩn
